...
void ChayAnimationGapKhuc(bool &dangChayAnimation, int &indexHanhDong, float &timeAccumulated,
...
int main()
{
    ...
    while (window.isOpen())
    {
                ...
                if (mouseButtonPressed->button == Mouse::Button::Left)
                {
                    ...
                    if (Start.getGlobalBounds().contains(mousePos))
                    {
                        ...
                        if (!ViTriSwap.empty())
                        {
                            ...
                            if (vt1 > vt2)
                                swap(vt1, vt2); // Đảm bảo vt1 luôn ở bên trái
        ...
        ChayAnimationGapKhuc(dangChayAnimation, indexHanhDong, timeAccumulated, DSachVienBi, ViTriSwap, vt1, vt2, posStart1, posStart2, speed, deltaTime);
        // ------------------------------------------------
                          ...
                          float speed, float deltaTime)
{
    ...
    if (!dangChayAnimation || indexHanhDong >= ViTriSwap.size())
        return;

    // Tích lũy thời gian thực đã trôi qua vào biến toàn cục/bên ngoài
    ...
    float doCao = 130.f;                            // Quãng đường đi dọc
    float doDichX = abs(posStart2.x - posStart1.x); // Quãng đường đi ngang thực tế

    // 2. TỰ ĐỘNG TÍNH THỜI GIAN CẦN THIẾT CHO TỪNG GIAI ĐOẠN (Tính bằng giây)
    // Thời gian = Quãng đường / Tốc độ (pixel / giây)
    float thoiGianGD1 = doCao / speed;
    float thoiGianGD2 = doDichX / speed;
    float thoiGianGD3 = thoiGianGD1; // Đi xuống bao nhiêu thì đi lên bấy nhiêu

    // Các mốc thời gian chuyển giai đoạn
    float moc1 = thoiGianGD1;
    float moc2 = moc1 + thoiGianGD2;
    float tongThoiGianCapNay = moc2 + thoiGianGD3;

    // 3. TÍNH TOÁN VỊ TRÍ DỰA TRÊN THỜI GIAN TÍCH LŨY
    // GIAI ĐOẠN 1: Tách ra theo trục Y
    ...
    // GIAI ĐOẠN 2: Chạy ngang qua nhau theo trục X
    ...
    // GIAI ĐOẠN 3: Chạy gập vào vị trí đích theo trục Y
    ...
    // Áp dụng tọa độ vào thực tế
    DSachVienBi[vt1].setPosition({x1, y1});
    DSachVienBi[vt2].setPosition({x2, y2});

    // 4. KHI HOÀN THÀNH ĐỦ THỜI GIAN CỦA CẶP NÀY
    if (timeAccumulated >= tongThoiGianCapNay)
    {
        swap(VienBi123[vt1], VienBi123[vt2]); // TRÁO ĐỔI TRONG MẢNG LOGIC
        // Ghim chặt tọa độ đích phòng trường hợp lệch vài pixel do sai số float
        ...
        // Qua bước tiếp theo
        indexHanhDong++;
        timeAccumulated = 0.f; // Reset bộ đếm thời gian về 0 để cặp sau dùng lại
